...
- **Stop points:** hard stop at the end of Phase 2 (persistence choice + repair policy are
  the user's to set). Do not implement repair without an explicit policy decision.
- **Phase-2 decisions (received):**
...
- **Pin = `apache-iceberg-1.10.2`** (recommended; user may override to 1.10.0 for exact paper-fidelity).
...
### The four questions the user asked
...
- **Version gap to resolve before Phase 3:** source read is Iceberg `main`; harness loads 1.6.1.
  ...
  bundle. Flag for the user; do not silently switch the harness engine. **[Superseded by Entry 3:
...
- `RowDataReader implements PartitionReader<InternalRow>` ⇒ **one reader instance per Spark task**.
  ...
  at different points of one stream. (This is the user's strengthening of the Phase-1 finding, and
  it holds: verified in source.)
...
**Flagged, not fixed (per instruction), because it leans on the same idea but is not directly
...
**Question (user):** does any release tag already have the post-refactor `FileRewriteRunner` structure?
...
- **Flag-off baseline re-run** should be on the **published `iceberg-spark-runtime-3.5_2.12:1.10.2`
  jar**, not `main` and not 1.6.1 — this obviates re-running on `main` entirely (the user's request was
...
- **Open risk before the re-run:** the harness driver (`iceberg_driver.py`) uses low-level Java writer
  ...
  Not launched yet — pending the pin confirmation, since if the user prefers 1.10.0 for exact
...
- **RECOMMENDED — metadata gate, then residual shuffle.** Iceberg manifests carry per-file
  ...
  - *Sound:* never skips a real violation (a violation implies an inversion). *Conservative:* may shuffle
    ...
    an over-approx of corruption — this is the honest version of the paper's property **for compute**, not
    just output. This gate is the interesting research bit and the reviewer-convincing answer.
- **Honest limitation to state in the paper:** even the gate is a *sound over-approximation*; a pathological
...
### Addition B — repair costing (honest, against the non-co-residency finding)
...
- **Why repair should NOT be a headline (recommend future work / guarded opt-in):**
  ...
  4. Atomicity: rewrite and repair are separate commits ⇒ a compacted-but-unrepaired window and partial-
     failure states.
- **Honest verdict:** repair is *feasible* and *can* scale with corruption, but (2) and (3) make it
...
not. Honest framing for the paper: report both, and separate the structural (STALE_WINS) from the
...
- 1.6.1's default rewrite still ends at 50→1 (strips them), so the **1.6.1 "keys vanish" behavior is
  ...
  orphans on 1.10.2 is now OPEN (expire won't — they're live in the current snapshot). Honest §5 framing
...
Chased per the user's flag. The generator produces a **variable** number of equality-delete files: one
...
**§5 framing recommendation (do NOT edit §5 unprompted — surface to user).** Split the two classes:
...
- Measurable on the harness: **soundness** (8-cell with gate on → still 5,440, nothing gated out) and the
  degenerate over-audit rate (≈100%, reported honestly as a limitation).
...
the initial "over-audit ~100%" was this bug, NOT the random-merge-overlap story. Honest correction.
...
- **Over-audit (headline):** on the random-merge harness the gate audits **9 of 9 groups** (all 8 violating
  ...
  across sequence numbers even on the clean table. This is the honest harness number, now for the right
...
- **Framing (per user):** lead with 100% over-audit → mechanism (random-merge LSN, pessimal by design) →
...
**The decisive property (the one the user most wanted):**
...
## Entry 20 — Phase 5: table-level merge feasibility (report BEFORE implementing, per user)
...
1. **Table-level merge (feasible).** Each group emits, for keys with discarded versions but no survivor
   ...
   intermediate candidate set grows. This is the honest characterization: it does not fully preserve the
...
3. **Accept the FN (do nothing).** Per-group detection is a sound lower bound (one-sided). Honest
...
than characterize it; its cost caveat (scales with straddling) must be stated. Decision is the user's.

## Entry 21 — Phase 5 DECISION (user) + framing constraints
...
scaling with straddling. Worth stating in the paper — it is the honest reason the complete mode is
strictly more expensive than the base one, not merely differently-scaled.

**Two modes, as the user framed them:**
...
Question (user): given Iceberg's defaults and realistic table sizes, what group counts do production
...
**Recommendation (analysis only — default NOT changed, per instruction).** Keep cross-group **opt-in**,
...
so they are the *worst case*, not the expected case. Stating that honestly matters: quoting 98% as a
...
**Cost of doing it correctly (state honestly):** statistics registration is a **separate commit** after the
...
- MAY NOT: any production overhead percentage. **Neither scale is data-dominated** — both baselines are
  ...
  where compaction runs for minutes, which this laptop harness cannot produce. The honest framing for the
...
**Scope note (honest):** this probe measures *layout geometry* (group counts and per-key partition
...
Reporting both brackets the answer honestly instead of picking a favourable point.
...
*commit-dominated* shape — and then extrapolated to a *row-dominated* one. The honest per-record ceiling
...
**FALSE ZERO CAUGHT (the pattern the user flagged).** Probe 1 first reported `log files: 0`, which is
...
### The checker story, honestly (measured vs inferred)
...
Source-and-docs characterization, per instruction: no sweep ported, no checker built. Two small probes
...
So the honest narrowness claim for the paper is: *the whole mechanism, including detection, the cost gate,
...
Note the honest refinement: at toy scale the gated arm measured **-2.9%** (indistinguishable from stock).
...
Design agreed with the user: the oracle must be derived from the generator's *construction*, never from
...
Still to add per the user: explicit duplicate injection (equal-sequence co-location, the FLINK-38450
...
honest answer is in between and resolves toward cloud. Eviction removed the *systematic* error, so §6.3 is
